...
**Scene Flow Diagram:** [02b-scenario-flow.puml](../diagrams/02b-scenario-flow.puml)

![Scenario Flow](../diagrams/02b-scenario-flow.png)
...
| 新频段 | 添加新的运营商频段 | 在 band_info 表添加新频段记录 |

## 6. PlantUML 图说明

### 01-scenario-view.puml (用例图)

场景视图用例图展示了系统的主要参与者、用例以及它们之间的关系。

**主要分组:**
- **NL2SQL 查询**: 自然语言转 SQL 查询相关用例
- **CQRS 数据查询**: 命令查询职责分离模式的数据查询用例
- **数据展示与交互**: 前端展示和报告生成用例
- **知识与协作**: RAG 知识查询和跨 Agent 协作用例

### 02b-scenario-flow.puml (场景流图)

场景流图详细展示了完整数据查询与可视化流程的数据流向。

**关键流程:**
1. 前端发起 HTTP 请求 (Query 或 NL2SQL)
2. API Proxy 路由到 operator-agent
3. Agent 根据意图路由到对应的 Controller
4. CQRS Command Side: NL2SQL Service 调用 SQLCoder 生成 SQL
5. CQRS Query Side: Query Services 直接从 Repository 读取数据
6. 数据逐级返回，最终在 Dashboard 中可视化
...
**数据流图参考:**
- [dfd-01-nl2sql-query.puml](../diagrams/dfd-01-nl2sql-query.puml) - NL2SQL 数据转换流
- [dfd-02-cqrs-query.puml](../diagrams/dfd-02-cqrs-query.puml) - CQRS Command/Query 数据流
- [dfd-03-system-overview.puml](../diagrams/dfd-03-system-overview.puml) - 系统整体数据流

**提示:** 使用 PlantUML 编辑器打开 `.puml` 文件可查看交互式图表，或导出为 PNG/PDF。