...
- **数据模型**: operator_info (运营商) + band_info (频段) + site_info (站点) + indicator_info (指标) + operator_summary (聚合)
...
operator-agent 新增智能后续问题推荐功能，在用户查询后自动生成相关建议问题:

- **上下文感知**: 根据用户查询类型 (站点/小区/指标/历史) 生成相关推荐
- **运营商名称**: 推荐问题包含实际数据库中的运营商名称
- **SSE 事件**: 通过 `type: 'followup'` 事件发送后续问题列表
- **前端组件**: `FollowupQuestions` 组件展示可点击的问题按钮

**示例**: 用户查询"北京联通有多少站点"后，推荐:
- "北京联通的小区数量是多少？"
- "北京联通各频段站点分布如何？"
- "北京联通最新指标数据有哪些？"

### OperatorSummary 物理站点统计

operator-service 新增 `operator_summary` 表和物理站点统计功能：

- **V2-V5 Migration**: 数据库迁移脚本完善，支持跨技术类型 null 值填充
- **Endpoint**: `/api/v1/query/summary/physical-sites` 获取物理站数/小区数统计
- **Entity**: `OperatorSummary`, `SiteSummary`, `IndicatorSummary` 实体类
- **Mapper**: 对应 MyBatis Mapper XML 实现

### Permission Cache 权限缓存

auth-agent 新增权限缓存优化，提升权限检查性能：

- **缓存策略**: 用户权限和角色信息缓存
- **数据范围控制**: 新增 `data_scope` 权限控制
- **审计日志**: 完整的 `audit_log` 表和查询接口
- **迁移脚本**: V1/V2/V3 迁移支持
