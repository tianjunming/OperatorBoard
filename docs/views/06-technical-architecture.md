...
agent-app 新增以下交互体验优化组件：

#### 智能图表推荐引擎 (`src/utils/chartRecommendation.js`)

| 函数 | 功能 |
|------|------|
| `hasTimeDimension()` | 检测时间维度 |
| `hasRatioMetrics()` | 检测比率指标 |
| `isPartToWhole()` | 检测占比关系 |
| `recommendChartType()` | 推荐最佳图表类型 |
| `getChartRecommendation()` | 获取推荐结果 |

#### KPI卡片组件 (`src/components/KpiCard.jsx`)

增强版KPI卡片，支持Sparkline迷你趋势图：
- 基于 Recharts AreaChart 实现
- 支持上升/下降/稳定趋势指示
- 点击展开详情（预留接口）

#### 模糊查询确认对话框 (`src/components/QueryConfirmationDialog.jsx`)

- 运营商/频段/月份选择
- 优雅的折叠面板设计
- 支持确认和取消操作

#### 骨架屏组件 (`src/components/SkeletonLoader.jsx`)
...
`useStreamingAgent` Hook 新增：
- `showConfirmation` - 确认对话框状态
