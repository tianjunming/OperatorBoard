...
![4+1视图总览](../docs/images/4p1_views.png)

![SDD认知迭代](../docs/images/sdd_evolution.png)

### 1.1 我对 SDD 的三次认知迭代

**第一次认知（2019年）：SDD 就是领域驱动设计**

![场景视图](../docs/images/scenario_view.png)
...
![传统软件工程 vs LLM-Based SDD 对比](../docs/images/对比表_传统vsLLM.png)
...
![Multi-Agent协作](../docs/images/multi_agent.png)
...
**【可复用 Skill：Multi-Agent 协作模式设计】**

```
Skill 7：Multi-Agent 架构设计三问
┌─────────────────────────────────────────────────────────────┐
│ 问题1：任务能否并行？                                         │
│   └─ 如果子任务相互独立 → Specialist Agent 并行执行            │
│   └─ 如果有依赖关系 → 按依赖顺序串行                           │
│                                                             │
│ 问题2：谁来汇总结果？                                        │
│   └─ 结果简单 → 直接返回给用户                                │
│   └─ 结果复杂 → Verifier Agent 校验 + 整合                   │
│                                                             │
│ 问题3：某个 Agent 失败了怎么办？                              │
│   └─ 降级策略：LLM 专家 → 规则引擎 → 预设回复                 │
│   └─ 熔断机制：连续失败 N 次 → 切换策略                       │
│                                                             │
│ 协作模式选择：                                               │
│   - Planner-Agent 模式：任务拆解 + 分发 + 汇总               │
│   - 流水线模式：上一个 Agent 输出作为下一个输入               │
│   - 广播模式：一个任务同时发给多个 Agent，取最优              │
└─────────────────────────────────────────────────────────────┘
```
...
![部署视图](../docs/images/deployment_view.png)
...
![意图识别架构图](../docs/images/intent_detection.png)

![意图识别方案对比](../docs/images/对比表_意图识别方案.png)
...
![RAG知识增强](../docs/images/rag_knowledge.png)
...
**【可复用 Skill：RAG 知识增强四步法】**

```
Skill 3：LLM 知识增强实施框架
┌─────────────────────────────────────────────────────────────┐
│ Step 1：知识分层                                            │
│   ├── 文档知识（Schema、API文档）     → LLM 知道有什么      │
│   ├── 领域知识（业务规则、计算公式）  → LLM 知道怎么算       │
│   ├── 系统知识（接口契约、调用模式）  → LLM 知道怎么用       │
│   └── 专业知识（运营商特定知识）      → LLM 知道边界在哪     │
│                                                             │
│ Step 2：向量化策略                                           │
│   - 表级 Chunk：每个表的 Schema + 注释                       │
│   - 指标级 Chunk：每个指标的 定义 + 公式 + 单位              │
│   - 领域级 Chunk：频段分类、运营商归属                       │
│                                                             │
│ Step 3：检索增强时机                                         │
│   - 查询分析阶段 → 注入表结构信息                            │
│   - SQL 生成阶段 → 注入字段映射                              │
...
**【可复用 Skill：LLM 输出异常排查三板斧】**

```
Skill 4：LLM 异常问题快速定位
┌─────────────────────────────────────────────────────────────┐
│ 第一斧：环境隔离验证                                         │
│   └─ 本地单独跑 Prompt，看输出是否正常                       │
│   └─ 排除网络、代理、日志等干扰因素                           │
│                                                             │
│ 第二斧：输入简化测试                                         │
│   └─ 把复杂输入逐步精简，看哪个触发了异常                     │
...
│                                                             │
│ 第三斧：换 LLM 对照                                         │
│   └─ 用另一个 LLM 跑同样的 Prompt                           │
│   └─ 如果另一个正常 → 当前 LLM 的特有问题                    │
│   └─ 如果另一个也异常 → Prompt 本身的问题                     │
│                                                             │
│ 排查优先级：环境 > 输入 > Prompt > LLM 特性                  │
│ 记住：有时候最简单的解法就是「换英文 Prompt」                  │
└─────────────────────────────────────────────────────────────┘
```
...
**【可复用 Skill：Prompt 模板安全写法】**

```
Skill 5：Prompt 模板规范
┌─────────────────────────────────────────────────────────────┐
│ 原则1：Prompt 占位符和代码占位符必须区分                      │
│   ✗ Python f-string: f"SELECT * FROM {table}"             │
│   ✗ Python format: "{table}".format(table="site_info")   │
│   ✓ Jinja2 模板: {{ table }}                               │
│                                                             │
│ 原则2：Prompt 中使用 {{ }} 表示字面大括号                     │
│   例：输出格式是 JSON → The format is {{"json"}}           │
│                                                             │
│ 原则3：示例代码块用 {% raw %}{% endraw %}包裹                 │
│   {% raw %}                                                 │
│   示例: {"intent": "{{intent}}"}                          │
│   {% endraw %}                                              │
│                                                             │
...
![SQL安全校验](../docs/images/sql_safety.png)
...
**【可复用 Skill：LLM 输出安全护栏设计】**

```
Skill 6：LLM 生成内容安全校验框架
┌─────────────────────────────────────────────────────────────┐
│ 核心原则：Prompt 约束是「建议」，代码校验是「法律」             │
│                                                             │
│ 校验层 1：语法安全（必须）                                    │
│   - 必须以 SELECT 开头（禁止 INSERT/DROP/DELETE）           │
│   - 必须包含 LIMIT（防止全表扫描）                           │
│   - 禁止注释（-- /* */）                                    │
│   - 禁止子查询嵌套超过 3 层                                   │
│                                                             │
...
![Command/Query类型对比](../docs/images/对比表_CommandQuery.png)

**Command与Query类型对比**（见上图）
...
**【可复用 Skill：LLM Provider 可切换架构】**

![LLM Provider抽象](../docs/images/llm_provider.png)

```
Skill 8：多 LLM Provider 抽象设计
┌─────────────────────────────────────────────────────────────┐
│ 核心思想：核心逻辑不依赖具体 LLM 实现                          │
│                                                             │
│ 抽象层设计：                                                 │
│   ┌─────────────────┐                                       │
│   │   LLMClient     │  ← 核心代码依赖这个接口                 │
│   │   (抽象接口)     │                                       │
│   └────────┬────────┘                                       │
│            │                                                │
│   ┌────────┴────────┬───────────────┐                      │
...
![SSE流式响应](../docs/images/sse_streaming.png)
...
│ 双重发送机制：                                               │
│   - 流式事件（type: row）→ 边收边渲染                        │
│   - 完成事件（type: done）→ 包含完整数据                      │
│                                                             │
│ 前端处理：                                                   │
│   EventSource 监听 → 状态机处理 → UI 更新                    │
│   用户点击导出 → 调用 done 事件的完整数据                    │
│                                                             │
│ 错误处理：                                                   │
│   - 连接断开 → 自动重试（有限次）                            │
│   - 超时未完成 → 降级为轮询                                  │
│   - 部分数据已到达 → 提供「导出已有数据」选项                │
│                                                             │
│ 适用场景：                                                   │
│   - 数据量大（>1000 条）                                     │
│   - 用户需要实时反馈                                         │
...
![开发视图](../docs/images/development_view.png)

![Prompt契约](../docs/images/prompt_contract.png)
...
![系统可靠性](../docs/images/reliability.png)
...
**【可复用 Skill：系统可靠性设计检查清单】**

```
Skill 10：LLM 系统可靠性自检清单
┌─────────────────────────────────────────────────────────────┐
│ 降级链路检查：                                               │
│   LLM 失败 → 规则引擎兜底 → 预设查询 → 友好错误              │
│   每一级都要测试，确保能正常切换                               │
│                                                             │
│ 熔断机制检查：                                               │
│   - 连续失败 N 次（如 5 次）→ 触发熔断                       │
...
**【可复用 Skill：LLM 项目上线检查清单】**

```
Skill 11：LLM 功能上线前检查
┌─────────────────────────────────────────────────────────────┐
│ 1. Prompt 版本化                                            │
│   □ 所有 Prompt 有版本号                                     │
│   □ 每次修改有 changelog 记录                                 │
...
│ 2. 降级链路测试                                              │
│   □ 模拟 LLM 超时 → 验证降级触发                             │
│   □ 模拟 LLM 返回错误格式 → 验证异常处理                     │
│   □ 模拟 LLM 返回恶意内容 → 验证安全护栏                     │
│                                                             │
│ 3. 日志埋点                                                 │
│   □ 请求 ID（串联全链路）                                    │
│   □ Prompt 版本号                                           │
...
![Skill索引表](../docs/images/skill_index.png)
...
║             │ Skill 8: 多 LLM Provider 抽象       │ 供应商锁定           ║
╠═════════════╪════════════════════════════════════╪═════════════════════╣
║ 意图识别     │ Skill 2: LLM与规则引擎级联架构      │ 不可解释性           ║
╠═════════════╪════════════════════════════════════╪═════════════════════╣
║ 知识管理     │ Skill 3: RAG 知识增强四步法         │ LLM 记性不好         ║
╠═════════════╪════════════════════════════════════╪═════════════════════╣
║ 开发调试     │ Skill 4: LLM 异常排查三板斧         │ 输出异常定位         ║
║             │ Skill 5: Prompt 模板安全写法        │ 占位符冲突           ║
╠═════════════╪════════════════════════════════════╪═════════════════════╣
║ 安全合规     │ Skill 6: LLM 输出安全校验框架       │ SQL 注入/全表扫描    ║
╠═════════════╪════════════════════════════════════╪═════════════════════╣
║ 架构设计     │ Skill 7: Multi-Agent 协作三问       │ Agent 编排           ║
║             │ Skill 9: 流式响应状态机设计          │ SSE 完整性           ║
╠═════════════╪════════════════════════════════════╪═════════════════════╣
║ 可靠性       │ Skill 10: 系统可靠性自检清单         │ 不知道测什么         ║
║             │ Skill 11: LLM 功能上线前检查         │ 上线遗漏             ║
╚═══════════════════════════════════════════════════════════════════════╝
```
...
![LLM演进路线](../docs/images/llm_evolution.png)
...
**这些 Skill 不是教条，是经验的凝练**。每个团队、每个项目的情况不同，你不必照搬全部，但希望这些 Skill 能在你遇到类似问题时，提供一个思考的方向。毕竟，**工程的本质就是站在前人的肩膀上，少走弯路**。
...
**Skill 实践延伸阅读**：
- [Jinja2 模板引擎文档](https://jinja.palletsprojects.com/) - Prompt 模板推荐工具
